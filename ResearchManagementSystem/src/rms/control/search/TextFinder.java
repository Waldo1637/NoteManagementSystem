...
public class TextFinder extends AbstractFinder {
    ...
    protected boolean accept(Item i) {
        ...
        if(Helpers.containsIgnoreCase(i.getName(), searchText)) return true;
        
        //if the item is a TextItem, check the text
        if(i instanceof TextItem){
            TextItem ti = (TextItem)i;
            if(ti.getText() == null) return false;
            
            return Helpers.containsIgnoreCase(ti.getText(), searchText);
        }
        return false;
    }
