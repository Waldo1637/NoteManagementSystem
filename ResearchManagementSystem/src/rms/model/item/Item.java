...
public abstract class Item implements Serializable {
    ...
    protected Item(ItemThread parentThread, String name){
        if(parentThread == null) throw new IllegalArgumentException("Parent thread cannot be null");
    ...
    public Item getParent(){
        ...
        return parentThread.get(pIndex);
    }
    
    public Item getChild(){
        int cIndex = parentThread.indexOf(this) + 1;
        if(cIndex >= parentThread.size()){
            return null;
        }
        return parentThread.get(cIndex);
    }
